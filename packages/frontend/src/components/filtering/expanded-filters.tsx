...
export type FilterContext = "landing" | "client" | "trainer";
...
export function ExpandedFilters({
    ...
    }, [isOpen, filters, context]);

    // Clear cached filters when context changes
    useEffect(() => {
        setFilters(null);
    }, [context]);
    ...
    const fetchFilters = async () => {
        ...
        setError(null);
        try {
            let filtersData: FiltersResponse;

            // Use appropriate endpoint based on context
            switch (context) {
                case "client":
                    filtersData = await filtersService.getClientFilters();
                    break;
                case "trainer":
                    filtersData = await filtersService.getTrainerFilters();
                    break;
                case "landing":
                default:
                    filtersData = await filtersService.getFilters();
                    break;
            }
            ...
            if (err instanceof Error) {
                ...
                if (err.message.includes("401") || err.message.includes("Unauthorized")) {
                    errorMessage = "No tienes permisos para acceder a estos filtros";
                } else if (err.message.includes("404")) {
                    errorMessage = "No se encontraron filtros disponibles";
                } else {
    ...
    return (
        <CustomModal isOpen={isOpen} onClose={onClose}>
            ...
            >
                ...
                {filters && !isLoading && (
                    <>
                        {/* Show context-specific message when no filters available */}
                        {filters.categories.length === 0 &&
                            filters.zones.length === 0 &&
                            filters.languages.length === 0 && (
                                <div className="bg-blue-50 border border-blue-200 text-blue-700 px-3 py-2 rounded text-sm">
                                    {context === "client" &&
                                        "No tienes servicios contratados para filtrar"}
                                    {context === "trainer" &&
                                        "No tienes servicios creados para filtrar"}
                                    {context === "landing" &&
