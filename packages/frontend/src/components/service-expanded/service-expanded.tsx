...
import { useCurrentService, useServicesLoading } from "@/store/services/services.hooks";
...
const ServiceExpanded = () => {
    ...
    const availableTimes = service?.availability
        ? service.availability
              .filter((av: { day: string; startTime: string }) => av.day === selectedDay)
    ...
    const hasHireServiceAndIsCancelledOrCompleted = myHirings.some((hiring) => {
        if (!service) return false;
        return (
            hiring.serviceId._id === service.id &&
            (hiring.status === "cancelled" || hiring.status === "completed")
        );
    });

    const clientCanBook =
        service?.trainerId !== user?.id &&
        (!hasHiredService || hasHireServiceAndIsCancelledOrCompleted);
                        ...
                        <header className="flex flex-col sm:flex-row items-start sm:items-center gap-4 border-b pb-6">
                            <div className="flex items-center gap-4">
                                <div
                                    ...
                                    onClick={() =>
                                        navigate(`/profile/${service.trainerId}`)
                                    }
                                >
