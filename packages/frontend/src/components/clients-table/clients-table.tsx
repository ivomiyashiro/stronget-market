...
const ClientsTable = () => {
  ...
  const [clientToAction, setClientToAction] = useState<ServiceClient | null>(
    null
  );
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      await hiringService.updateHiringStatus(
        clientToAction.hiringId,
        newStatus
      );
  ...
  const filesButtonDisabled = (client: ServiceClient) => {
    return client.status !== "completed" && client.status !== "confirmed";
  };
                ...
                <TableRow key={client.hiringId}>
                  ...
                  <TableCell>
                    <div className="text-sm">
                      <div className="font-medium">{client.day}</div>
                      <div className="text-gray-500">{client.time}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <span
                  ...
                  <TableCell>
                    <Button
                      ...
                      disabled={filesButtonDisabled(client)}
                    >
                    ...
                    <div className="flex gap-2">
                      {client.status === "confirmed" && (
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => handleCompleteService(client)}
                      ...
                      {(client.status === "pending" ||
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => handleCancelService(client)}
