...
export class ServicesService {
    ...
    async getClientServices(
        ...
        const hirings = await Hiring.find(hiringQuery).populate({
            path: "serviceId",
            select: "category description duration price mode zone language availability trainerId visualizations isActive status",
            populate: {
                path: "trainerId",
        ...
        const validHirings = hirings.filter((hiring: any) => hiring.serviceId !== null);
