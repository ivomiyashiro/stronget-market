...
export class HiringService {
  async createHiring(
    ...
    const requestedDateTime = this.parseDateTime(
      hiringData.day,
      hiringData.time
    );
    ...
    try {
      ...
      if (
        ...
        return {
          canBook: false,
          reason:
            "Formato de hora inválido, por favor selecciona una hora válida",
        };
      }
      ...
      if (requestedDateTime <= minBookingTime) {
        return {
          canBook: false,
          reason:
            "La reserva debe ser como mínimo 2 horas en adelante, por favor selecciona una hora válida",
        };
      }
