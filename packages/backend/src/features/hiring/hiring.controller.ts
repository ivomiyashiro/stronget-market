...
export class HiringController {
  ...
  getTrainerHirings = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(400).json({
        message: error instanceof Error ? error.message : "Get trainer hirings failed",
      });
    }
  };
  ...
  updateHiringStatus = async (
    ...
    try {
      ...
      const hiring = await this.hiringService.updateHiringStatus(id, status, userId);
      res.status(200).json(hiring);
    } catch (error) {
      res.status(400).json({
        message: error instanceof Error ? error.message : "Update hiring status failed",
      });
    }
  };
